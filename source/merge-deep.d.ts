...
To merge two elements we go through them and check the presence of each key. If the key exists only in the destination, returns the destination. If the key exists only in the source, returns the source. But if the key exists in both elements, a merge mode must be chosen.

There are 4 modes for merging objects properties that you can set with the {@link MergeDeepOptions.recordMergeMode recordMergeMode} option (default: `merge-or-replace`):
...
There are 6 modes for merging arrays elements that you can set with the {@link MergeDeepOptions.arrayMergeMode arrayMergeMode} option (default: `spread`):
- `merge-or-replace`: Try to merge the values for that key, otherwise replace the destination value by the source value.
